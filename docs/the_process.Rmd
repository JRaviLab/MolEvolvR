...
prot_opinscls <- read_lines("data/rawdata_opinscls/psp.op_ins_cls", #data/rawdata_opinscls/prot.op_ins_cls
                   n_max=-1, na = "NA")
...
DA.cumulative <- total_counts(prot.DA.summ.byLin, cutoff =0, type = "DA")
DA.cumulative %>%
  head() %>%
  kable()

GC.cumulative <- total_counts(prot.GC.summ.byDALin, cutoff = 50, type = "GC") 
GC.cumulative %>%
  head() %>%
  kable()
...
lineage.DA.plot(prot, DA.cumulative,"DomArch.norep", "")

lineage.DA.plot(prot, GC.cumulative,"GenContext.norep", "")
```
