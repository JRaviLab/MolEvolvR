...
prot <- read_tsv("data/rawdata_tsv/all_raw_query_uniqop.txt", col_names = c("AccNum", "ClustID", "ClustName.orig", "GenContext.orig", "DomArch.Pfam","DomArch.orig","-", "Length", "GeneName", "Lineage", "Species.orig", "GCA_ID", "Annotation", "GI")) %>% select(-"-", -GI,-Annotation)
...
prot <- prot %>%
  cleanup_clust(repeat2s = TRUE,remove_tails = FALSE, domains_keep, domains_rename)
