...
# Rename DUF-1707 to SHOCT-bihelicca
# all$DomArch <- purrr::map(all$DomArch, function(x)str_replace_all(x, pattern = "DUF1707-SHOCT", "SHOCT-bihelical" )) %>% unlist()
# all$DomArch.repeats <- purrr::map(all$DomArch.repeats, function(x)str_replace_all(x, pattern = "DUF1707-SHOCT", "SHOCT-bihelical" ))%>% unlist()
# all$GenContext.repeats <- purrr::map(all$GenContext.repeats, function(x)str_replace_all(x, pattern = "DUF1707-SHOCT", "SHOCT-bihelical" ))%>% unlist()
# all$GenContext <- purrr::map(all$GenContext, function(x)str_replace_all(x, pattern = "DUF1707-SHOCT", "SHOCT-bihelical" ))%>% unlist()


DUF1700 <- all %>% filter(grepl("(?<!-)ahelical",ignore.case = T, DomArch, perl = T))
DUF1707 <- all %>% filter(grepl("SHOCT-bihelical",ignore.case = T, DomArch))
...
toast_rack = all%>%filter(grepl("Toast_rack",ignore.case = T, DomArch))
