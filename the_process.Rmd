...
#    source("R/",i)
#  }
source_files <- list.files("R/")
map(paste0("R/", source_files), source)
...
#write_tsv(x=prot, path="data/rawdata_tsv/all_raw.txt", col_names=T)
...
prot <- prot %>%
  cleanup_clust(domains_keep=domains_keep,
                domains_rename=domains_rename,
                repeat2s=TRUE,
                remove_tails=F, #new! check below if it works!
                remove_empty=F) #new! check below if it works!

prot <- prot %>%
  cleanup_domarch(domains_keep=NULL, # filter applied to only ClustName for now.
                domains_rename=domains_rename,
...
prot <- prot %>%
  remove_tails(by_column="ClustName")
...
# write_tsv(x=prot, path="data/rawdata_tsv/all_semiclean.txt", col_names=T)
```
