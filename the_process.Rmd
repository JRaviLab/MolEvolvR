...
# Post-cleanup
#colnames_prot <- c("AccNum", "ClustID", "ClustName.orig", "GenContext.orig","DomArch.Pfam", "DomArch.orig", "-", "Length", "GeneName", "Lineage", "Species.orig", "GCA_ID", "Annotation", "GI")
prot <- read_tsv(here("data/rawdata_tsv/all_semiclean.txt"), col_names= T) #colnames_prot)
#prot_uniq <- read_tsv(here("data/rawdata_tsv/all_uniqgc.txt"), col_names=colnames_prot)
#prot_uniq <- prot_uniq %>% cleanup_gencontext(domains_rename=domains_rename, repeat2s=T)
#prot$GenContext.uniq <- prot_uniq$GenContext
...
# Per ...?
##!! Is this reading ClustName or DomArch?
upset.plot(dropdown, 99, "da2doms")
```

```{r upset-plot-gc, eval=T, out.height="600px"}
upset.plot(dropdown, 90, "gc2da")
```
...
##!! Is this reading ClustName or DomArch?
wordcloud_element("da2doms", dropdown, cutoff = 100) 
wordcloud_element("gc2da", dropdown, cutoff = 70)
```
...
##!! Is this reading ClustName or DomArch?
DA.cumulative <- total_counts(dropdown,"DomArch",
                              cutoff =100)
DA.cumulative %>%
  head() %>%
  kable()

GC.cumulative <- total_counts(dropdown,"GenContext",
                              cutoff=50) 
GC.cumulative %>%
  head() %>%
  kable()
...
lineage.DA.plot(dropdown, 
                colname="ClustName", type="da2doms", cutoff=80)

lineage.DA.plot(dropdown,
                "GenContext", type="gc2da", cutoff=40)
```
...
domain_network(dropdown, "ClustName",
               domains_of_interest="pspa|snf7", cutoff_type="Total Count",
               100, "circle")
...
seq_tree(paste0(here("data/alns//"), dropdown_var, ".fa"))
