...
countByColumn <- function(prot = prot, column = "DomArch", min.freq = 1) {
    ...
    counts <- prot %>%
        ...
        filter(!grepl("^-$", .data[[column]])) %>% # remove "-"
...
words2WordCounts <- function(string) {
    ...
    df_word_count <- string %>%
        ...
        filter(!grepl(" \\{n\\}", .data$words)) %>%
        filter(!grepl("^c\\($", .data$words)) %>% # remove "c("
        filter(!grepl("^\\)$", .data$words)) %>% # remove ")"
        filter(!grepl("^-$", .data$words)) %>% # remove "-"
        filter(!grepl("^$", .data$words)) %>% # remove empty rows
        filter(!grepl("^\\?$", .data$words)) %>% # remove "?"
        filter(!grepl("^\\?\\*$", .data$words)) %>% # remove "?*"
        filter(!grepl("^tRNA$", .data$words)) %>% # remove "tRNA"
        filter(!grepl("^ncRNA$", .data$words)) %>% # remove "ncRNA"
        filter(!grepl("^rRNA$", .data$words)) %>% # remove "rRNA"
        filter(!grepl("^X$|^X\\(s\\)$", .data$words)) %>% # remove "X" and "X(s)"
...
totalGenContextOrDomArchCounts <- function(prot, column = "DomArch", lineage_col = "Lineage",
    ...
    col_count <- prot %>%
        group_by({{ column }}) %>%
        summarise(totalcount = sum(.data$count))
    ...
    total <- total %>%
        ...
        arrange(-.data$totalcount, -.data$count)
